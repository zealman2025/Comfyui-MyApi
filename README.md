...
| 节点 | 输入图片 | 输出 | 主要特性 | 适用场景 |
|------|----------|------|----------|----------|
| 🌐 BizyAir Gemini3Flash VLM | 1张 | 文本 | 图像理解、自动文本描述生成、支持系统提示词和用户提示词 | 图像分析、内容描述、图像标注 |
...
### 🖼️ 图像输入优化
- **单图分析**: 连接到第一个图像输入
- **对比分析**: 使用多个图像输入进行对比（适用于支持多图的节点）
- **图像格式**: 支持PNG、JPEG、WEBP等常见格式
- **分辨率**: 建议使用适中分辨率，避免过大图像
- **VLM节点**: BizyAir Gemini3Flash VLM会自动处理图像压缩和编码转换
...
### 🔍 VLM图像分析工作流
1. 使用**BizyAir Gemini3Flash VLM**节点：输入图像、系统提示词和用户提示词
2. 节点会自动下载并解析API返回的文本文件
3. 输出纯文本结果，可直接连接其他文本处理节点
...
6. **VLM节点显示乱码**: 节点已自动处理UTF-8编码，如仍有问题请检查网络连接
7. **VLM节点显示URL而非文本**: 节点会自动下载文本文件，如显示URL请检查网络连接和API响应
